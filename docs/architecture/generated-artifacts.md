...
The Phase-03 worker protocol has approved project-owned schema and protobuf/protoc 33.1 generation inputs. The generated OR-Tools source contract now records the approved source-stage pin, matched protobuf input, protocol digest, worker identity, and cross-target CMake decisions. Native builders, installed solver manifests, artifact SBOMs/license payloads, packaging, and release evidence remain separately gated and must use measured outputs rather than placeholders.
