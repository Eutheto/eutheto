...
`just worker-build-native` downloads the generated fixed-hash source set, builds the approved Windows x86_64 worker with MSVC/Ninja and the static `MultiThreaded` runtime, runs its native tests, recursively derives and inspects the x64 project DLL closure, rejects every runtime import outside the exact Windows system set or packaged project closure, and verifies EOF startup. Rust then verifies reviewed license-source hashes, installs the exact license and project NOTICE payload, generates and validates the artifact-specific SPDX SBOM and solver manifest, and replaces `.cache/ortools-native/windows-x86_64/current` only after staging validation. A failed pre-publication rerun preserves the last-good result, and an interrupted replacement is recovered on the next locked build. The command requires network access for fixed archives and does not use an ambient solver or protobuf. Manifest-validated `worker-smoke`, the Tauri sidecar, clean-machine installer, Windows arm64, signing, and release publication remain gated.
